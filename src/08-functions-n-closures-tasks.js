...
function getPolynom(...args) {
  if (args.length === 3) {
    return (y) => args[0] * y ** 2 + args[1] * y + args[2];
  }
  if (args.length === 2) {
...
function logger(func, logFunc) {
  return (...values) => {
    const valueStr = values.map((value) => JSON.stringify(value));
    logFunc(`${func.name}(${valueStr}) starts`);
    const result = func.apply(this, values);
    logFunc(`${func.name}(${valueStr}) ends`);
    return result;
  };
}
